...
game_exit_rect = pygame.Rect(500, 450, 60, 60)

gameover_exit_rect = pygame.Rect(0, 0, 60, 60)  # sẽ set lại trong draw_game_over
    ...
    while True:
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if rule_exit_rect.collidepoint(event.pos):
                    return "back"
...
def draw_menu(screen, font, btn_font):
    ...
    draw_button_base(screen, menu_exit_rect, icon=icon_exit)
    draw_button_base(screen, rule_exit_rect, icon=icon_rule)
